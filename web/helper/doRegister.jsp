...
<%
	...
	{
		flag++;
		response.sendRedirect("../register.jsp?msg=Length Must Between 5 and 15 Characters!");
	}
	...
	{
		flag++;
		response.sendRedirect("../register.jsp?msg=Password Must be in AlphaNumeric!");
	}
	...
	{
		flag++;
		response.sendRedirect("../register.jsp?msg=Password and Confirmation Password Must be Same!");
	}
	/*else if(alphaName == 0)
	{
		flag++;
	...
	{
		flag++;
		response.sendRedirect("../register.jsp?msg=Phone Number Must be Filled!");
	}
	else if(check == 0)
	{
		flag++;
		response.sendRedirect("../register.jsp?msg=Phone Number Must be Numeric!");
	}
	else if(phone.length()<5)
	{	
		flag++;
		response.sendRedirect("../register.jsp?msg=Phone Number Minimum Length is 8!");
	}
	/*else if(contains==0)
	{
		flag++;
	...
	{
		flag++;
		response.sendRedirect("../register.jsp?msg=Wrong Email Format!");
	}
	
	if(flag == 0)
	{
		String query = "insert into members values ( '"+userID+"' , '"+name+"' , '-' , '-' ,'"+role+"' , '"+phone+"', '"+email+"', '-' ,'"+date+"', '-' , '-' ,'"+password+"', '"+confirm+"' , '-', '-', '-' ,'"+location+"','-','-')";
		st.executeUpdate(query);
		response.sendRedirect("../index.jsp?msg=Registration Complete");
	}
